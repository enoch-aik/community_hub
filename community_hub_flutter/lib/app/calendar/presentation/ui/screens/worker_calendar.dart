...
@RoutePage(name: 'workerCalendar')
class WorkerCalendarScreen extends HookConsumerWidget {
  const WorkerCalendarScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final appointments = ref.watch(workerBookingStreamProvider(
          ...
          child: appointments.when(
              data: (data) {
                return getScheduleViewCalendar(
                    ...
                    context: context,
                    sources:
                        data.bookings.toSyncfusionAppointment(isWorker: true));
              },
              error: (e, _) {
                return getScheduleViewCalendar(
                    context: context,
